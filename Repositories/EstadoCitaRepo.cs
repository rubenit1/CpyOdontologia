...
using DTOs.EstadoCita;

public sealed class EstadoCitaRepo
{
    private readonly string _cs;
    private const string SP = "dbo.sp_crud_estado_cita";

    public EstadoCitaRepo(IConfiguration cfg) => _cs = cfg.GetConnectionString("DefaultConnection")!;
    ...
    // C
    public async Task<(bool ok, string mensaje, int? codigo)> CrearAsync(EstadoCitaCreateDto dto)
    {
    ...
    // R (solo activos)
    public async Task<IEnumerable<EstadoCitaDto>> ListarActivosAsync()
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryAsync<EstadoCitaDto>(
            SP,
    ...
    // I (por id)
    public async Task<EstadoCitaDto?> ObtenerAsync(int id)
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryFirstOrDefaultAsync<EstadoCitaDto>(
            SP,
    ...
    // U
    public async Task<(bool ok, string mensaje, int? codigo)> ActualizarAsync(int id, EstadoCitaUpdateDto dto)
    {
        using var conn = new SqlConnection(_cs);
        var pm = await conn.QueryFirstOrDefaultAsync<ProcMsg>(
            SP,
