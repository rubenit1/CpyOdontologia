...
public sealed class MotivoCitaRepo
{
    private readonly string _cs;
    private const string SP = "dbo.sp_crud_motivo_cita";

    public MotivoCitaRepo(IConfiguration cfg) => _cs = cfg.GetConnectionString("DefaultConnection")!;
    ...
    // C
    public async Task<(bool ok, string mensaje, int? codigo)> CrearAsync(MotivoCitaCreateDto dto)
    {
    ...
    // R (solo activos)
    public async Task<IEnumerable<MotivoCitaDto>> ListarActivosAsync()
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryAsync<MotivoCitaDto>(
            SP,
    ...
    // I (por id)
    public async Task<MotivoCitaDto?> ObtenerAsync(int id)
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryFirstOrDefaultAsync<MotivoCitaDto>(
            SP,
    ...
    // U
    public async Task<(bool ok, string mensaje, int? codigo)> ActualizarAsync(int id, MotivoCitaUpdateDto dto)
    {
        using var conn = new SqlConnection(_cs);
        var pm = await conn.QueryFirstOrDefaultAsync<ProcMsg>(
            SP,
