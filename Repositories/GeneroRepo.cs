...
public sealed class GeneroRepo
{
    private readonly string _cs;
    private const string SP = "dbo.sp_crud_genero";

    public GeneroRepo(IConfiguration cfg) => _cs = cfg.GetConnectionString("DefaultConnection")!;
    ...
    // C
    public async Task<(bool ok, string mensaje, int? codigo)> CrearAsync(GeneroCreateDto dto)
    {
    ...
    // R (solo activos)
    public async Task<IEnumerable<GeneroDto>> ListarActivosAsync()
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryAsync<GeneroDto>(
            SP,
    ...
    // I (por id)
    public async Task<GeneroDto?> ObtenerAsync(int id)
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryFirstOrDefaultAsync<GeneroDto>(
            SP,
    ...
    // U
    public async Task<(bool ok, string mensaje, int? codigo)> ActualizarAsync(int id, GeneroUpdateDto dto)
    {
        using var conn = new SqlConnection(_cs);
        var pm = await conn.QueryFirstOrDefaultAsync<ProcMsg>(
            SP,
