...
public sealed class RolRepo
{
    private readonly string _cs;
    private const string SP = "dbo.sp_crud_rol";

    public RolRepo(IConfiguration cfg) => _cs = cfg.GetConnectionString("DefaultConnection")!;
    ...
    // C
    public async Task<(bool ok, string mensaje, int? codigo)> CrearAsync(RolCreateDto dto)
    {
    ...
    // R (solo activos)
    public async Task<IEnumerable<RolDto>> ListarActivosAsync()
    {
        using var conn = new SqlConnection(_cs);
    ...
    // I (por id)
    public async Task<RolDto?> ObtenerAsync(int id)
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryFirstOrDefaultAsync<RolDto>(
            SP,
    ...
    // U
    public async Task<(bool ok, string mensaje, int? codigo)> ActualizarAsync(int id, RolUpdateDto dto)
    {
        using var conn = new SqlConnection(_cs);
        var pm = await conn.QueryFirstOrDefaultAsync<ProcMsg>(
            SP,
