...
public sealed class TipoSangreRepo
{
    private readonly string _cs;
    private const string SP = "dbo.sp_crud_tipo_sangre";

    public TipoSangreRepo(IConfiguration cfg) => _cs = cfg.GetConnectionString("DefaultConnection")!;
    ...
    // C
    public async Task<(bool ok, string mensaje, int? codigo)> CrearAsync(TipoSangreCreateDto dto)
    {
    ...
    // R (solo activos)
    public async Task<IEnumerable<TipoSangreDto>> ListarActivosAsync()
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryAsync<TipoSangreDto>(
            SP,
    ...
    // I (por id)
    public async Task<TipoSangreDto?> ObtenerAsync(int id)
    {
        using var conn = new SqlConnection(_cs);
        return await conn.QueryFirstOrDefaultAsync<TipoSangreDto>(
            SP,
    ...
    // U
    public async Task<(bool ok, string mensaje, int? codigo)> ActualizarAsync(int id, TipoSangreUpdateDto dto)
    {
        using var conn = new SqlConnection(_cs);
        var pm = await conn.QueryFirstOrDefaultAsync<ProcMsg>(
            SP,
