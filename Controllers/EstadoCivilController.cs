...
public sealed class EstadoCivilController : ControllerBase
{
    private readonly EstadoCivilRepo _repo;
    public EstadoCivilController(EstadoCivilRepo repo) => _repo = repo;

    [HttpGet] // activos
    public async Task<ActionResult<IEnumerable<EstadoCivilDto>>> GetActivos()
        => Ok(await _repo.ListarActivosAsync());

    [HttpGet("{id:int}")]
    public async Task<ActionResult<EstadoCivilDto>> Get(int id)
        => (await _repo.ObtenerAsync(id)) is { } x ? Ok(x) : NotFound();

    [HttpPost]
    public async Task<IActionResult> Post([FromBody] EstadoCivilCreateDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Put(int id, [FromBody] EstadoCivilUpdateDto dto)
    {
