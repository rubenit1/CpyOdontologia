...
public sealed class MedicamentoCatalogoController : ControllerBase
{
    private readonly MedicamentoCatalogoRepo _repo;
    public MedicamentoCatalogoController(MedicamentoCatalogoRepo repo) => _repo = repo;

    [HttpGet] // activos
    public async Task<ActionResult<IEnumerable<MedicamentoCatalogoDto>>> GetActivos()
        => Ok(await _repo.ListarActivosAsync());

    [HttpGet("{id:int}")]
    public async Task<ActionResult<MedicamentoCatalogoDto>> Get(int id)
        => (await _repo.ObtenerAsync(id)) is { } x ? Ok(x) : NotFound();

    [HttpPost]
    public async Task<IActionResult> Post([FromBody] MedicamentoCatalogoCreateDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Put(int id, [FromBody] MedicamentoCatalogoUpdateDto dto)
    {
