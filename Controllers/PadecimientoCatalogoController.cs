...
public sealed class PadecimientoCatalogoController : ControllerBase
{
    private readonly PadecimientoCatalogoRepo _repo;
    public PadecimientoCatalogoController(PadecimientoCatalogoRepo repo) => _repo = repo;

    [HttpGet] // activos
    public async Task<ActionResult<IEnumerable<PadecimientoCatalogoDto>>> GetActivos()
        => Ok(await _repo.ListarActivosAsync());

    [HttpGet("{id:int}")]
    public async Task<ActionResult<PadecimientoCatalogoDto>> Get(int id)
        => (await _repo.ObtenerAsync(id)) is { } x ? Ok(x) : NotFound();

    [HttpPost]
    public async Task<IActionResult> Post([FromBody] PadecimientoCatalogoCreateDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Put(int id, [FromBody] PadecimientoCatalogoUpdateDto dto)
    {
