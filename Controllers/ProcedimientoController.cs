...
public sealed class ProcedimientoController : ControllerBase
{
    private readonly ProcedimientoRepo _repo;
    public ProcedimientoController(ProcedimientoRepo repo) => _repo = repo;

    [HttpGet] // activos
    public async Task<ActionResult<IEnumerable<ProcedimientoDto>>> GetActivos()
    ...
    [HttpPost]
    public async Task<IActionResult> Post([FromBody] ProcedimientoCreateDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Put(int id, [FromBody] ProcedimientoUpdateDto dto)
    {
