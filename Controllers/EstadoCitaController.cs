...
public sealed class EstadoCitaController : ControllerBase
{
    private readonly EstadoCitaRepo _repo;
    public EstadoCitaController(EstadoCitaRepo repo) => _repo = repo;

    [HttpGet] // activos
    public async Task<ActionResult<IEnumerable<EstadoCitaDto>>> GetActivos()
        => Ok(await _repo.ListarActivosAsync());

    [HttpGet("{id:int}")]
    public async Task<ActionResult<EstadoCitaDto>> Get(int id)
        => (await _repo.ObtenerAsync(id)) is { } x ? Ok(x) : NotFound();

    [HttpPost]
    public async Task<IActionResult> Post([FromBody] EstadoCitaCreateDto dto)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Put(int id, [FromBody] EstadoCitaUpdateDto dto)
    {
